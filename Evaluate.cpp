...
int Evaluate::evaluateGame(){
    ...
    for(int i=16;i<48;i++){
        ...
        case 31:
        case 47:value+=evaluete_K(i);break;     //后面的数转按位与16==0
        }
    }
    return value;
}
...
int Evaluate::evaluete_N(int PieciesId){
    ...
    int value = _N;
    for(int i=0;i<8;i++){
        int Dst = Pieces[PieciesId]+KnightMoveTab[i];   //目标
        int Lag = Pieces[PieciesId]+HorseLegTab[i];     //腿
        if(InBoard[Dst]==0 || Board[Lag]!=0){     //如果出了棋盘或者马腿了
            //什么也不做
            continue;
        }else if(Board[Dst]==0){
            value+=Mobility_N;//加机动性
        }else if(isSameColor(PiecesInBoard[Dst],PieciesId)){    //如果目标位置和改棋子一样
            //保子价值
            switch (Board[Dst]) {
...
bool Evaluate::Check(bool isRedturn){
    ...
    //向上
    for(int i=Pieces[PiecesId]-16;InBoard[i]!=0;i-=16){ //将视为车往上检查
