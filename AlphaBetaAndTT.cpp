...
int AlphaBetaAndTT::NegaMax(int depth, int alpha, int beta){
    ...
    int value;
    //查找置换表获取的值
    if(TT->lookUpTT(Zobrist,depth,value,alpha,beta)){ //如果存在相同情况(不改变zobrist)
        nDistance--;
        return value;
    }
    //叶子节点hash
    if(depth<=0){
        ...
        //存确切值
        TT->storeToTT(Zobrist,depth,value,TranspositionTable::exact); //不改变初始zobrist
        nDistance--;
        return value;
    }
    QVector<Step*> steps;
...
void AlphaBetaAndTT::showStep(Step* step, int value, bool flag){
    int time_begin=clock();
