...
void IDAlphaBeta::getBestStep(int* _Board,int* _Pieces,int* _PiecesInBoard,bool computerColor,int timeLimit,QVector<Step*>&Steps){
    ...
    //迭代搜索
    for(nDepth=1;nDepth<=maxDepth;nDepth++){        //越往深越好?
         time_begin_IDS=clock();
        //如果没有超时,保存最佳着法
        if(NegaMax(nDepth,-LARGENUMBER,LARGENUMBER)==66666){
            break;
        }
        *backupBestMove=*bestStep;      //明明返回都一样为啥不能直接
    }
...
int IDAlphaBeta::NegaMax(int depth, int alpha, int beta){
    ...
    //每一次迭代都开始一次新的搜索并重新开始计时
    //检测是否超时
    int time_now=clock();
    if((time_now-time_begin_IDS)/1000>=timeLimit){
        if(nDistance%2==0){     //判断边
            return 66666;
        }else{
            return -66666;
        }
    ...
    //遍历走棋
    for(auto iter=steps.begin();iter!=steps.end();iter++){      //历史表可以对其排序
        fakeMove(*iter);
...
void IDAlphaBeta::fakeMove(Step *step){
    ...
    MoveSide=!MoveSide;
    TT->getFakeMoveZobrist(step,Zobrist);   //改变zob了
...
void IDAlphaBeta::backFakeMove(Step *step){
    ...
    MoveSide=!MoveSide;
    TT->getbackFakeMoveZobrist(step,Zobrist);   //改变zob了
    movedSteps.removeLast();
}

//检查重复局面(0正常，1和棋，2长将输棋)
int IDAlphaBeta::checkRepeat(){
    //moveSteps包括人类和电脑走过的,以及电脑搜索时虚拟走过的
    ...
    if(checkNum>=1){
        ...
        Step* step = movedSteps.back(); //返回step中2最后一个元素的引用
        //长捉
        QVector<Step*> steps;
        ...
        case 8:MGR.getSteps_R(step->Dst_Position,steps);break;  //红车，所有可能的走法存入这个step
        case 9:MGR.getSteps_N(step->Dst_Position,steps);break;  //红马，8个可能走法存入
        case 10:MGR.getSteps_C(step->Dst_Position,steps);break; //红炮
