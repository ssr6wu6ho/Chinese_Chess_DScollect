...
bool MoveControler::CanMove(int piecesId, int DstBoard){
    ...
    switch (Board[Pieces[piecesId]]) {  //棋子ID对应函数操作
...
bool MoveControler::CanMoveN(int piecesId, int DstBoard){
    ...
    for(int i=0;i<8;i++){//判断是否是马步及是否撇马脚
        if((Pieces[piecesId]+KnightMoveTab[i])==DstBoard&&
            Board[Pieces[piecesId]+HorseLegTab[i]]==0){
            return true;
        }
    }
    return false;
...
bool MoveControler::CanMoveB(int piecesId, int DstBoard){
    ...
    for(int i=0;i<4;i++){
        if((Pieces[piecesId]+ElephantMoveTab[i])==DstBoard&&
            Board[Pieces[piecesId]+ElephantEyeTab[i]]==0){
            return true;
        }
    }
    return false;
...
bool MoveControler::CanMoveP(int piecesId, int DstBoard){
    ...
    if((piecesId&32)!=0){
        if(DstBoard==Pieces[piecesId]+16){
            return true;
        }else if((DstBoard==Pieces[piecesId]+1 || DstBoard==Pieces[piecesId]-1) &&
                   (DstBoard&0x80)!=0){
            return true;
        }
